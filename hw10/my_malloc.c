...
static metadata_t* split_block(metadata_t* block, size_t size) {
    size_t tot = size + TOTAL_METADATA_SIZE;
    metadata_t* newAdd = (metadata_t*)((uint8_t*)block + (block->size) - size);
    block->size -= tot;
    newAdd->size = size;
    return newAdd;
}
...
void *my_malloc(size_t size) {
    ...
    metadata_t* newNode = find_best_fit(size);
    ...
    if (newNode == NULL) {
        ...
        newNode = find_best_fit(size);
    }

    if (newNode->size - size >= MIN_BLOCK_SIZE) {
        newNode = split_block(newNode, size);
        return newNode + 1;
    }
    else {
        remove_from_addr_list(newNode);
        return newNode + 1;
    }
...
void my_free(void *ptr) {
    ...
    metadata_t* p = (metadata_t*)((uint8_t*)ptr - TOTAL_METADATA_SIZE);
    metadata_t* right = find_right(p);
    metadata_t* left = find_left(p);
    if (left != NULL) {
...
void *my_realloc(void *ptr, size_t size) {
    ...
    metadata_t* newNode = my_malloc(size);
    if (newNode == NULL) {
        return NULL;
    }

    memcpy(newNode, ptr, size);
    my_free(ptr);
    return newNode;
...
void *my_calloc(size_t nmemb, size_t size) {
    ...
    metadata_t* newNode = my_malloc(tot);
    if (newNode == NULL) {
        return NULL;
    }
    newNode = memset(newNode, 0, tot);
