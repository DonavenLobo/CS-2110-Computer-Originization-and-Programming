...
static metadata_t* find_right(metadata_t* freed_block) {
    ...
    while (curr != NULL) {
        if ((uintptr_t)freed_block == (uintptr_t)((uint8_t*)(curr - 1) - (freed_block->size))) {
            return curr;
        }
        curr = curr->next;
    }
...
static metadata_t* split_block(metadata_t* block, size_t size) {
    metadata_t* temp = (metadata_t*)((uint8_t*)block + (block->size) - size);
    temp->size = size;
...
static metadata_t* find_best_fit(size_t size) {
    ...
    // stores the best imperfectly sized block
    metadata_t* imperfectly_sized_block = NULL;
    long unsigned int imperfect_size;

    metadata_t* curr = address_list;
    while (curr != NULL) {
        // if perfect size, return
        // else if imperfectly sized block is NULL, set imperfectly sized block
        // else if imperfectly sized block bigger than sized block
        if (curr->size == size) {
            return curr;
        }
        else if (curr->size > size && imperfectly_sized_block == NULL) {
            imperfectly_sized_block = curr;
            imperfect_size = curr->size;
        }
        else if (curr->size > size && (curr->size < imperfect_size)) {
            imperfectly_sized_block = curr;
            imperfect_size = curr->size;
        }
        curr = curr->next;
    }
    return imperfectly_sized_block;
}
...
void* my_malloc(size_t size) {
    ...
    // if node available, return out of memory if SBRK returns -1
    // else if node available, allot more memory with SBRK
    if (node == NULL) {
        ...
        metadata_t* left = find_left(temp);
        if (left != NULL) {
            merge(left, temp);
        }
    ...
    if (node->size >= size + TOTAL_METADATA_SIZE + 1) {
        metadata_t* newNode = split_block(node, size);
        return newNode + 1;
    }
    else {
        // if (node -> size == size) {
...
void my_free(void* ptr) {
    ...
    metadata_t* right = find_right(temp);
    metadata_t* left = find_left(temp);

    // left only
    // left then right merge
    // right only
...
void* my_realloc(void* ptr, size_t size) {
    ...
    metadata_t* temp = my_malloc(size);
    if (temp == NULL) {
        return NULL;
    }
...
void* my_calloc(size_t nmemb, size_t size) {
    ...
    metadata_t* curr = my_malloc(nmemb * size);

    if (curr == NULL) {
        return NULL;
    }

    curr = memset(curr, 0, nmemb * size);
